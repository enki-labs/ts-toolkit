...
class Sparkplot:
    ...
    """
    def __init__(self, type='line', data=None, input_file="data.txt", output_file="",
    ...
    def plot_sparkline(self):
        ...
        """
        close() #clear previous plots

        #if len(self.data) == 0:
        #    self.data = self.get_input_data()
        num_points = len(self.data)
        min_data = self.data.min() #min(self.data)
        max_data = self.data.max() #max(self.data)
        #sum_data = sum(self.data)
        #avg_data = sum(self.data) / num_points
        min_index = self.data.argmin() #self.data.index(min_data)
        max_index = self.data.argmax() #self.data.index(max_data)
        ...
        fig_width = min(2.2, max(1.5, 0.03 * num_points))
        ...
        fig = figure(1, figsize=(fig_width, fig_height), dpi=600)
        ...
        axis([-1, num_points, 0.99*min_data, 1.01*max_data ])                
